...
**Status:** Draft — planning only, no hooks wired yet.
...
- Use `language: system` so the hook shells out to the venv's own `black`/`ruff` (already pinned in `backend/requirements-dev.txt`) rather than a hook-managed duplicate toolchain — keeps one source of truth for tool versions and guarantees hook output matches CI output.
...
- **[CONCERN — project-reviewer, 2026-07-24] Adding these as "new coverage" inverts the plan's own "mirror, don't replace" principle** — `bot-ci` (`ci.yml:114-139`) runs only `pytest tests/ -v`, no lint step, so the hook would be the *only* enforcement surface for bot/ formatting/lint. A contributor without hooks enabled can land badly-formatted bot/ code and CI stays green. **Resolution:** treat closing the CI gap as a prerequisite, not a nice-to-have — file a small companion change adding `black --check app/ tests/` + `ruff check app/ tests/` to the `bot-ci` job (mirrors the backend job's two steps exactly) before or alongside Phase 1, so the bot/ hook is a genuine mirror like every other area instead of an unmatched extension.
